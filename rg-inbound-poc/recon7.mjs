...
// 입고 카드 내부의 "다음 >" → 이후 푸터 "다음" (최대 4회 반복해 3단계까지)
for (let round = 0; round < 4; round++) {
  ...
  const state = await dumpStructure(page, `19-after-next-r${round}`);
  log(`round ${round} 이후 URL:`, state.url);
  const step3Reached = await page.evaluate(() => { const el = [...document.querySelectorAll('[class*=step], [class*=Step]')].find(e => (e.innerText||'').includes('물류센터/일정 확정') && (e.className||'').match(/active|current|selected/i)); const cal = document.querySelector('[class*=calendar], [class*=Calendar]'); return !!(el || cal); });
  if (step3Reached) {
    log('*** 3단계(물류센터/일정) 도달! ***');
